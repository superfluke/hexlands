...
public class VanillaBiomeProvider extends BiomeProvider
{
    ...
    protected VanillaBiomeProvider()
    {
        this.biomeCache = new BiomeCache(this);
        this.biomesToSpawnIn = Lists.newArrayList(allowedBiomes);
    }

    private VanillaBiomeProvider(long seed, WorldType worldTypeIn, String options)
    {
        this();
    ...
    public VanillaBiomeProvider(WorldInfo info)
    {
    ...
    public Biome[] viableList(int x, int z, int radius)
    {
        IntCache.resetIntCache();
        int i = x - radius >> 2;
        int j = z - radius >> 2;
        ...
        int j1 = l - j + 1;
        int[] aint = this.genBiomes.getInts(i, j, i1, j1);
        Biome[] blist = new Biome[i1*j1];
        for (int k1 = 0; k1 < i1 * j1; ++k1)
        {
            blist[k1] = Biome.getBiome(aint[k1]);
        }
